...
import { useCreateDesarrollador } from "../mutations";
import { useNotification } from "@/components/useNotifications";
import { useEffect } from "react";
import { formatearRut, limpiarRut } from "@/utils/rutChileno";

type CrearDesarrolladorModalProps = {
  isOpen: boolean;
  onClose: () => void;
};

export default function CrearDesarrolladorModal({
  isOpen,
  onClose,
}: CrearDesarrolladorModalProps) {
  ...
  const { mutate: crearDev, isPending } = useCreateDesarrollador();
  ...
  const onSubmit = (data: FormData) => {
    ...
    crearDev(payload, {
      onSuccess: () => {
        notifySuccess("Desarrollador creado con éxito");
        reset();
        onClose();
      },
      onError: () => notifyError("Error al crear"),
    });
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Crear nuevo desarrollador</DialogTitle>
          <DialogDescription>
            Completa todos los campos para registrar un desarrollador.
          </DialogDescription>
        </DialogHeader>
  ...
  <div>
    ...
    {errors.aniosExperiencia && (
      <p className="text-sm text-red-500">
        {errors.aniosExperiencia.message}
      </p>
    )}
  </div>
